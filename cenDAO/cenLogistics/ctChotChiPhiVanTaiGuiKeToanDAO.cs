                    ...
                    using (SqlTransaction sqlTransaction = sqlConnection.BeginTransaction())
                    {
                        using (SqlCommand sqlCommand = new SqlCommand(ctChotChiPhiVanTaiGuiKeToan.insertProcedureName, sqlConnection, sqlTransaction))
                        {
                            ...
                            foreach (ctChotChiPhiVanTaiGuiKeToan obj in objList)
                            {
                                ...
                                if (!NhatKyDuLieuDAO.Insert(cenCommon.cenCommon.AllPropertiesAndValues(obj), obj.ID, obj.ID, null, cenCommon.ThaoTacDuLieu.Them, cenCommon.cenCommon.TraTuDien(ctChotChiPhiVanTaiGuiKeToan.tableName), sqlConnection, sqlTransaction))
                                {
                    ...
                    using (SqlTransaction sqlTransaction = sqlConnection.BeginTransaction())
                    {
                        using (SqlCommand sqlCommand = new SqlCommand(ctChotChiPhiVanTaiGuiKeToan.deleteProcedureName, sqlConnection, sqlTransaction))
                        {
                            foreach (ctChotChiPhiVanTaiGuiKeToan obj in objList)
                            {
                                ...
                                sqlCommand.Parameters.AddRange(sqlParameters);
                                int rowAffected = sqlCommand.ExecuteNonQuery();
                                if (!NhatKyDuLieuDAO.Insert(cenCommon.cenCommon.AllPropertiesAndValues(obj), obj.ID, null, null, cenCommon.ThaoTacDuLieu.Xoa, cenCommon.cenCommon.TraTuDien(ctChotChiPhiVanTaiGuiKeToan.tableName), sqlConnection, sqlTransaction))
                                {
                                    sqlTransaction.Rollback();
                                    return false;
                                }
